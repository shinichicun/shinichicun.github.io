...
$$
最后我们直接去解这个方程就可以得到素数p，进而得到flag；这里用到了**精度域RealField(1000)**，所以可以不用考虑small_roots()需要求几位（个人理解，仅供参考）
...
> 先定义一下key：
> $$
> key=a^6*v0 + a^5*v1 + a^4*v2 + a^3*v3 + a^2*v4 + a*v5 + v6
> $$
> 我们这里假设：$(a*x)\oplus v=a*x+b,b\in[-255,255]$
>
...
> $$
> 对这个格进行调平，进行LLL，得到$\begin{pmatrix}b_0,b_1,\cdots,b_6,1,0\end{pmatrix}$的可能结果；然后在这些结果中，找到末尾是 **[1，0]** 的那一组，便得到了$\begin{pmatrix}b_0,b_1,\cdots,b_6\end{pmatrix}$

有了b后，我们回到刚刚假设的式子：
$$
1000003x_{i-1}+b=x_{i}=1000003x_{i-1}\oplus v_{i-1}
$$
刚刚的假设也不是白设的，因为我们根据这个式子，可以有这样的思路：

> 因为我们手上有：$x_{7}=Hash(key)\oplus 7$ 和 $A=1000003x_{6}=x_{7}-b_6$；
>
> 所以有：$v_{6}=x_{7}\oplus A$，$x_6=A*{1000003}^{-1}~mod~2^{64}$
>
> 那么依次类推，我们便有这样的循环：
>
> $A=1000003x_{6}=x_{7}-b_6$
>
> $v_{i-1}=x_{i}\oplus A$
>
> $x_{i-1}=A*{1000003}^{-1}~mod~2^{64}$
